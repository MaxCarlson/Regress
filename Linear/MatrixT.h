...
struct Traits<MatrixT<Type, MajorOrder>>
{
...
public:
	...
	inline friend std::ostream& operator<<(std::ostream& out, const MatrixT& m);
...
template<class Type, bool MajorOrder>
inline MatrixT<Type, MajorOrder>::MatrixT(size_type nrows, size_type ncols) :
	nrows{ nrows },
	ncols{ ncols },
	vals(nrows * ncols)
{}
...
template<class Type, bool MajorOrder>
inline MatrixT<Type, MajorOrder>::MatrixT(const std::initializer_list<std::initializer_list<Type>>& m) :
...
template<class Type, bool MajorOrder>
inline Type & MatrixT<Type, MajorOrder>::operator()(size_type row, size_type col)
{
	return MajorOrder ? vals[col * nrows + row] : vals[row * ncols + col];
}

template<class Type, bool MajorOrder>
inline const Type & MatrixT<Type, MajorOrder>::operator()(size_type row, size_type col) const
{
	return MajorOrder ? vals[col * nrows + row] : vals[row * ncols + col];
}

template<class Type, bool MajorOrder>
inline bool MatrixT<Type, MajorOrder>::operator==(const MatrixT& other) const
{
	if (rows() != other.rows()
		|| cols() != other.cols())
		return false;
...
template<class Type, bool MajorOrder>
template<class Func>
inline void MatrixT<Type, MajorOrder>::unaryExpr(Func && func)
{
...
template<class Type, bool MajorOrder>
template<class Func>
inline void MatrixT<Type, MajorOrder>::unaryExpr(Func && func) const
{
	for (int i = 0; i < size(); ++i)
		func(vals[i]);
}
...
template<class Type, bool MajorOrder>
template<class Func>
inline void MatrixT<Type, MajorOrder>::unaryExprPara(Func && func)
{
...
template<class Type, bool MajorOrder>
template<class Func>
inline void MatrixT<Type, MajorOrder>::unaryExprPara(Func && func) const
{
#pragma omp parallel for
	for (int i = 0; i < size(); ++i)
		func(vals[i]);
...
template<class Type, bool MajorOrder>
inline void MatrixT<Type, MajorOrder>::resize(size_type numRows, size_type numCols)
{
	nrows = numRows;
	ncols = numCols;
...
template<class Type, bool MajorOrder>
inline Type MatrixT<Type, MajorOrder>::sum() const
{	// TODO: Test unrolling
	Type sum = 0;
	for (int i = 0; i < size(); ++i)
		sum += vals[i];
...
inline void MatrixT<Type, MajorOrder>::addColumn(size_type idx, Type val)
{
	...
	MatrixT<Type, MajorOrder> tmp(nrows, ncols + 1);
...
template<class Type, bool MajorOrder>
inline std::ostream & operator<<(std::ostream & out, const MatrixT<Type, MajorOrder>& m)
{
	using Mat = MatrixT<Type, MajorOrder>;
